...
export default function Game({ game, onUpdate, user, onLogout }) {
  ...
  function touchCrystal() { if (crystalTouched) return; const positive = Math.random() >= 0.5; const result = { score: positive ? 15 : -15, message: positive ? 'La boule révèle une vision lumineuse de ton avenir.' : 'Une ombre inquiétante apparaît dans la vision.' }; setCrystalTouched(true); const next = { ...game, score: game.score + result.score, crystalResult: result }; onUpdate(next); saveGame(next); }
